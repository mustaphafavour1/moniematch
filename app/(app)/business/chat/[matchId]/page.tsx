...
export default function BizChatConvPage() {
  ...
  const menuItems = [
    ...
    { icon: 'clipboard', label: 'Send report',          action: () => { setMenuOpen(false); router.push(`/business/chat/${matchId}/send-report`) } },
                  ...
                  return (
                    <div key={m.id} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
                      ...
                      <div style={{ maxWidth: '72%' }}>
                        {isSpecial ? (
                          <BizSpecialBubble msg={m} isMine={isMine} matchId={matchId} />
                        ) : (
                          <div style={{
                            padding: '10px 14px',
...
function BizSpecialBubble({ msg, isMine, matchId }: { msg: ChatMessage; isMine: boolean; matchId: string }) {
  const router   = useRouter()
  ...
  async function open() {
    ...
    if (!isReport) {
      router.push(`/business/chat/${matchId}/offer-view?offerId=${msg.ref_id}&mode=view`)
      return
    }
  ...
  return (
    <>
      <button onClick={open} style={{
        ...
        <span style={{ fontSize: 20 }}>{isReport ? '📋' : '💰'}</span>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: isMine ? '#fff' : 'var(--ink)' }}>
            {isReport ? 'Report' : 'Offer'}
          </div>
