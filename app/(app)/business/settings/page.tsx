...
import { getSettings, saveSettings, setBusinessVisibility } from '@/lib/auth'
...
export default function BizSettingsPage() {
    ...
    Promise.all([
      ...
      import('@/lib/supabase').then(({ supabase }) =>
        supabase.auth.getUser().then(async ({ data: { user } }) => {
          ...
          const { data: biz } = await supabase.from('businesses').select('is_visible')
            .eq('owner_id', profileId).maybeSingle()
          return biz
        })
      ),
    ]).then(([raw, biz]) => {
      ...
      setSettings({
        ...DEFAULTS,
        ...base,
        privacy_show_profile: biz?.is_visible ?? DEFAULTS.privacy_show_profile,
      })
    })
  }, [])
  ...
  const toggle = async (key: keyof Settings) => {
    ...
    try {
      ...
      if (key === 'privacy_show_profile') {
        await setBusinessVisibility(next.privacy_show_profile)
      }
      setSaved(true)
      ...
      <div className="scroll" style={{ flex: 1 }}>
        <div className="pad" style={{ paddingTop: 16, paddingBottom: 40 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
            <ToggleRow label="Investor interest" sub="When an investor views or matches with you"
              value={settings.notif_new_investors} onToggle={() => toggle('notif_new_investors')} />
            <ToggleRow label="New messages" sub="When an investor sends you a message"
              value={settings.notif_new_messages} onToggle={() => toggle('notif_new_messages')} />
            <ToggleRow label="Deal signed" sub="When an investor confirms a deal"
              value={settings.notif_deal_signed} onToggle={() => toggle('notif_deal_signed')} last />
          </div>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <ToggleRow label="Show my profile to investors" sub="Investors can discover and view your listing"
              value={settings.privacy_show_profile} onToggle={() => toggle('privacy_show_profile')} last />
          </div>
