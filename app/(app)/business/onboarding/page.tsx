    ...
    try {
      ...
      if (user) {
        ...
        if (profile?.role === 'investor') {
          ...
          setError('This is an investor account. Sign in on the investor side.')
          setBusy(false)
          return
        }
      }
    ...
    try {
      ...
      if (user) {
        const { error: e } = await supabase.from('users').upsert(
          { id:user.id, name, username, role:'business_owner', email:user.email ?? null } as never,
          { onConflict:'id' }
        )
        if (e) throw e
      }
    ...
    try {
      ...
      if (user && !skip) {
        const { error: e } = await supabase.from('businesses').upsert(
          { owner_id:user.id, name:bizName, category, owner_name:name } as never,
          { onConflict:'owner_id' }
        )
        if (e) throw e
      }
        ...
        {mode === 'signin' && (
          <div className="screen-enter" style={{display:'flex', flexDirection:'column', height:'100%', padding:'64px 22px 28px'}}>
            ...
            <div style={{display:'flex', gap:8, marginBottom:24}}>
              <span className="chip forest">Business</span>
              <span className="chip outline">Sign in</span>
            </div>
            <div className="h2" style={{marginBottom:8}}>Welcome back</div>
            <p style={{color:'var(--ink-2)', fontSize:14, margin:'0 0 20px'}}>Sign in to your business account.</p>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:10}}>
              ...
              <div style={{textAlign:'center', fontSize:13, color:'var(--ink-3)', marginTop:4}}>
                ...
                <span onClick={() => { setMode('signup'); setError(''); setStep(1) }}
                  style={{color:'var(--forest)', fontWeight:600, cursor:'pointer'}}>
                  Create one
                </span>
              </div>
            </div>
        ...
        {mode === 'signup' && step === 3 && (
          <div className="screen-enter" style={{display:'flex', flexDirection:'column', height:'100%', padding:'64px 22px 28px'}}>
            <div className="row between" style={{marginBottom:24}}>
              <button onClick={() => setStep(2)} className="btn btn-soft"
            ...
            <div className="col gap-12">
              <div>
                ...
                <input value={bizName} onChange={e=>setBizName(e.target.value)}
                  ...
                  style={{border:'1px solid var(--line-strong)', background:'var(--bone)', borderRadius:14,
                    padding:'12px 14px', fontFamily:'var(--font-body)', fontSize:15, color:'var(--ink)',
            ...
            {error && <p style={{fontSize:13, color:'var(--clay)', margin:'12px 0 0', fontWeight:500}}>{error}</p>}

            <div style={{flex:1}} />
            <div className="col gap-8">
              <button className="btn btn-forest btn-block"
                disabled={!bizName||!category||busy}
                onClick={() => handleSaveBiz(false)}>
