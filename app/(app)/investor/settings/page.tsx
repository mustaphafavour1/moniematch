...
const PREF_KEY = 'mm_inv_settings'
      ...
      <div className="scroll" style={{ flex: 1 }}>
        <div className="pad" style={{ paddingTop: 16, paddingBottom: 40 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
            <ToggleRow
              label="New matches"
              sub="When a business matches your preferences"
              value={settings.notif_new_matches}
              onToggle={() => toggle('notif_new_matches')}
            />
            <ToggleRow
              label="New messages"
              sub="When a business sends you a message"
              value={settings.notif_new_messages}
              onToggle={() => toggle('notif_new_messages')}
            />
            <ToggleRow
              label="Monthly reports"
              sub="When an investee submits a report"
              value={settings.notif_reports}
              onToggle={() => toggle('notif_reports')}
              last
            />
          </div>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <ToggleRow
              label="Allow businesses to message me"
              sub="Business owners can start a conversation"
              value={settings.privacy_can_contact}
              onToggle={() => toggle('privacy_can_contact')}
              last
            />
          </div>
