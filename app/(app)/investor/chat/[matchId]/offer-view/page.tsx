...
function Chip({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ background: 'var(--cream)', borderRadius: 12, border: '1px solid var(--line)',
...
// ─── Counter offer form (same pattern as business side) ──────────────────────
...
export default function InvOfferViewPage() {
  ...
  // Bottom bar content for view mode
      ...
      bottomContent = (
        <div style={{ display: 'flex', gap: 10 }}>
          <button style={{ flex: 1, padding: '13px 0', borderRadius: 12, border: '1px solid var(--ink)',
            ...
            onClick={() => router.push(`/investor/chat/${matchId}/offer-view?offerId=${offerId}&mode=counter`)}>
            Counter offer
          </button>
  ...
  const showBottomBar = mode === 'view' || counterSent
