...
export default function SignInPage() {
  ...
  const routeByRole = async () => {
    ...
    if (!data?.role) {
      // User exists in Auth but hasn't completed onboarding — let them choose
      setNoRole(true)
      setBusy(false)
      return
    }
  ...
  const handleSignIn = async () => {
    ...
    setBusy(true); setError(''); setNoRole(false)
    try {
      ...
      else if (msg.includes('email not conf')) setError('Check your email to confirm your account first.')
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--ink-3)', marginTop: 6 }}>Sign in with your username or email</p>
        </div>

        {noRole ? (
          /* User authed but has no role yet — prompt them to complete onboarding */
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            <p style={{ fontSize: 15, color: 'var(--ink-2)', marginBottom: 24, lineHeight: 1.6 }}>
              Your account isn&apos;t set up yet.<br />Choose how you&apos;re joining:
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <a href="/investor/onboarding" style={{
              ...
              <a href="/business/onboarding" style={{
                display: 'block', padding: '14px', borderRadius: 14, textAlign: 'center',
                background: 'var(--forest)', color: '#fff', fontWeight: 700, fontSize: 15,
                textDecoration: 'none', fontFamily: 'var(--font-body)',
              }}>Join as Business Owner</a>
            </div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 28, fontSize: 13, color: 'var(--ink-3)' }}>
          New to MonieMatch?{' '}
          <a href="/investor/onboarding" style={{ color: 'var(--clay)', fontWeight: 600, textDecoration: 'none' }}>Join as Investor</a>
          {' · '}
          <a href="/business/onboarding" style={{ color: 'var(--forest)', fontWeight: 600, textDecoration: 'none' }}>Join as Business</a>
        </div>
      </div>
    </div>
  )
